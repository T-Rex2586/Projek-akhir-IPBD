"""
Skrip operasional untuk melatih ulang konfigurasi hiperparameter serta menyimpan artefak model pengklasifikasi anomali secara persisten.
"""
import os
import sys
import time
...
def fetch_training_data(hours: int=168) -> pd.DataFrame:
    ...
    try:
        ...
        prices = session.query(PriceData).filter(PriceData.timestamp >= since).order_by(PriceData.timestamp.asc()).all()
        if len(prices) > 50:
            df = pd.DataFrame([{'symbol': p.symbol, 'price': p.price, 'volume': p.volume or 0, 'timestamp': p.timestamp} for p in prices])
            logger.info('training_data_fetched_from_price_data', rows=len(df))
            return df
        klines = session.query(KlineData).filter(KlineData.open_time >= since).order_by(KlineData.open_time.asc()).all()
        if klines:
            df = pd.DataFrame([{'symbol': k.symbol, 'price': k.close_price, 'volume': k.volume, 'timestamp': k.open_time} for k in klines])
...
def train_model(contamination: float=0.05, hours: int=168) -> dict:
    ...
    metadata = {'trained_at': datetime.utcnow().isoformat(), 'training_samples': len(features), 'feature_count': features.shape[1], 'contamination': contamination, 'anomalies_detected': anomaly_count, 'anomaly_percentage': round(anomaly_pct, 2), 'training_hours': hours, 'model_path': MODEL_PATH}
    joblib.dump(metadata, METADATA_PATH)
    duration = time.time() - start_time
    summary = {'status': 'completed', 'duration_seconds': round(duration, 2), 'training_samples': len(features), 'anomalies_found': anomaly_count, 'normal_found': normal_count, 'anomaly_percentage': round(anomaly_pct, 2), 'model_saved_to': MODEL_PATH}
