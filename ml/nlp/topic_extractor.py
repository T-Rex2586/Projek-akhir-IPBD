...
def extract_topics(hours: int = 24, top_n: int = 15) -> List[Dict]:
    """
    Extract top N narrative topics from news articles in the last `hours`.
    Returns list of dicts: {'topic': str, 'weight': float, 'sentiment': float}
    """
    session = get_session()
    try:
        ...
        for idx in top_indices:
            ...
            # Estimate sentiment for this word by averaging sentiment of docs containing it
            # To do this quickly, we find which documents have non-zero tfidf for this word
            ...
            # Filter out generic words if needed, or too short
            if len(word) > 2 and weight > 0:
                results.append({
                    "topic": word,
                    "weight": round(weight * 100, 2),  # scale for easier plotting
