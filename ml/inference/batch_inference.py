"""
Modul inferensi komputasi terdistribusi (batch) untuk evaluasi skor anomali menggunakan model hutan isolasi (Isolation Forest) pada himpunan data tersimpan.
"""
import os
import sys
...
class BatchAnomalyInference:
    ...
    def score_price_data(self, hours: int=24) -> dict:
        ...
        try:
            ...
            df = pd.DataFrame([{'price': p.price, 'volume': p.volume or 0, 'symbol': p.symbol, 'timestamp': p.timestamp} for p in prices])
            ...
            for idx in anomaly_indices:
                row = df.iloc[idx]
                anomaly = {'event_type': 'batch_ml_anomaly', 'symbol': row.get('symbol', 'N/A'), 'description': f"Batch ML anomaly: price=${row['price']:,.2f}, volume={row['volume']:,.0f}, price_change={row['price_change']:.4f}", 'severity': 'medium', 'value': float(row['price']), 'threshold': 0}
                if save_anomaly_event(anomaly):
                    anomalies_saved += 1
            metrics.increment('anomalies_detected', anomalies_saved)
            summary = {'status': 'completed', 'total_scored': len(features), 'anomalies_detected': len(anomaly_indices), 'anomalies_saved': anomalies_saved, 'anomaly_rate': round(len(anomaly_indices) / len(features) * 100, 2)}
...
class BatchSentimentInference:
    ...
    def score_unscored_news(self, limit: int=500) -> dict:
        ...
        try:
            ...
            unscored = session.query(NewsArticle).filter(NewsArticle.sentiment_score == None).limit(limit).all()
            scored = 0
