"""
Modul inferensi sekuensial yang menerapkan arsitektur Long Short-Term Memory (LSTM) guna memproyeksikan lintasan harga serta sinyal perdagangan taktis.
"""
import sys
import os
import argparse
...
def fetch_recent_data(symbol: str, hours: int=6) -> pd.DataFrame:
    ...
    try:
        ...
        if not klines:
            ...
            return pd.DataFrame()
        df = pd.DataFrame([{'timestamp': k.close_time, 'close_price': k.close_price, 'volume': k.volume} for k in klines])
        df['hour'] = pd.to_datetime(df['timestamp']).dt.floor('h')
        keywords = {'BTCUSDT': ['bitcoin', 'btc'], 'ETHUSDT': ['ethereum', 'eth'], 'BNBUSDT': ['binance', 'bnb'], 'SOLUSDT': ['solana', 'sol'], 'ADAUSDT': ['cardano', 'ada']}
        kws = keywords.get(symbol.upper(), [symbol[:3].lower()])
        from sqlalchemy import or_, func
        sentiment_query = session.query(func.date_trunc('hour', NewsArticle.published_at).label('hour'), func.avg(NewsArticle.sentiment_score).label('avg_sentiment')).filter(NewsArticle.published_at >= since, or_(*[NewsArticle.title.ilike(f'%{kw}%') for kw in kws])).group_by(func.date_trunc('hour', NewsArticle.published_at))
...
def run_inference_loop(symbol: str, interval: int=300, alert_on_signal: bool=True):
    print(f"\n{'=' * 60}")
            ...
            try:
                ...
                if alert_on_signal and prediction['signal'] != last_signal:
                    if prediction['signal'] in ['BUY', 'SELL'] and prediction['confidence'] > 0.5:
                        ...
                        send_prediction_alert(symbol=symbol, current_price=prediction['current_price'], predicted_price=prediction['predicted_price'], signal=prediction['signal'], confidence=prediction['confidence'])
                        save_anomaly_event({'event_type': f"lstm_{prediction['signal'].lower()}_signal", 'symbol': symbol, 'description': f"LSTM prediction: {prediction['signal']} signal (predicted change: {prediction['price_change_pct']:+.2f}%)", 'severity': 'medium' if prediction['confidence'] < 0.7 else 'high', 'value': prediction['predicted_price'], 'threshold': prediction['current_price']}, send_alert=False)
                        logger.info('trading_signal_generated', symbol=symbol, signal=prediction['signal'], confidence=prediction['confidence'])
...
def main():
    parser = argparse.ArgumentParser(description='LSTM inference for trading signals')
    parser.add_argument('--symbol', type=str, default='BTCUSDT', help='Crypto symbol (default: BTCUSDT)')
    parser.add_argument('--interval', type=int, default=300, help='Prediction interval in seconds (default: 300)')
    parser.add_argument('--once', action='store_true', help='Make single prediction and exit')
    parser.add_argument('--no-alerts', action='store_true', help='Disable Telegram alerts')
    args = parser.parse_args()
    if args.once:
        predict_once(args.symbol)
    else:
        run_inference_loop(symbol=args.symbol, interval=args.interval, alert_on_signal=not args.no_alerts)
