"""
Analisis sinyal berita menggunakan ekstraksi kuantitatif guna menghubungkan fluktuasi sentimen media massa dengan dinamika perilaku harga pasar.
"""
import sys
import os
import time
...
class NewsSignalAnalyzer:
    ...
    def get_symbol_from_news(self, title: str, content: str) -> list:
        ...
        keywords = {'BTCUSDT': ['bitcoin', 'btc', 'satoshi'], 'ETHUSDT': ['ethereum', 'eth', 'ether', 'vitalik'], 'BNBUSDT': ['binance', 'bnb', 'binance coin'], 'SOLUSDT': ['solana', 'sol'], 'ADAUSDT': ['cardano', 'ada']}
        found_symbols = []
    ...
    def analyze_news_impact(self, news: NewsArticle) -> dict:
        ...
        if sentiment_score > 0.5:
            ...
            sentiment = 'neutral'
            urgency = 'low'
        breaking_keywords = ['breaking', 'urgent', 'alert', 'crash', 'surge', 'skyrocket', 'plunge', 'rally', 'milestone', 'record', 'hack', 'regulation', 'sec', 'etf', 'approved', 'rejected', 'ban']
        ...
        if sentiment_score > 0.6:
            ...
            recommendation = 'HOLD'
        return {'sentiment': sentiment, 'score': sentiment_score, 'urgency': urgency, 'symbols': symbols, 'recommendation': recommendation, 'title': title, 'source': news.source, 'published_at': news.published_at}
    ...
    def get_lstm_prediction(self, symbol: str) -> dict:
        ...
        try:
            ...
            klines = session.query(KlineData).filter(KlineData.symbol == symbol, KlineData.close_time >= since).order_by(KlineData.close_time.asc()).all()
            session.close()
            if not klines or len(klines) < 60:
                return None
            df = pd.DataFrame([{'timestamp': k.close_time, 'close_price': k.close_price, 'volume': k.volume, 'sentiment_score': 0.0} for k in klines])
    ...
    def combine_signals(self, news_analysis: dict, lstm_prediction: dict=None) -> dict:
        ...
        if lstm_prediction is None:
            return {'final_signal': news_rec, 'confidence': abs(news_sentiment) * 100, 'reason': 'Based on news sentiment only', 'lstm_available': False}
        ...
        return {'final_signal': final_signal, 'confidence': confidence, 'reason': reason, 'lstm_available': True, 'lstm_signal': lstm_signal, 'lstm_confidence': lstm_confidence, 'price_change_pct': price_change}

    def send_trading_alert(self, news_analysis: dict, combined_signal: dict, symbol: str):
        if not _is_configured():
            logger.debug('telegram_not_configured')
            return
        signal_emojis = {'STRONG BUY': '🟢🟢', 'BUY': '🟢', 'STRONG SELL': '🔴🔴', 'SELL': '🔴', 'HOLD': '🟡'}
        ...
        text = f"{emoji} <b>TRADING SIGNAL - {symbol}</b>\n━━━━━━━━━━━━━━━━━━\n{urgency_emoji} <b>Urgency:</b> {urgency}\n\n📰 <b>News Alert</b>\n<b>Source:</b> {news_analysis['source']}\n<b>Title:</b> {news_analysis['title'][:120]}...\n<b>Sentiment:</b> {news_analysis['sentiment'].title()} ({news_analysis['score']:+.2f})\n\n🎯 <b>RECOMMENDATION: {combined_signal['final_signal']}</b>\n💪 <b>Confidence:</b> {combined_signal['confidence']:.0f}%\n📝 <b>Reason:</b> {combined_signal['reason']}\n"
        if combined_signal.get('lstm_available'):
            text += f"\n🤖 <b>LSTM Analysis</b>\nSignal: {combined_signal['lstm_signal']}\nPredicted Change: {combined_signal['price_change_pct']:+.2f}%\nML Confidence: {combined_signal['lstm_confidence']:.0%}\n"
        text += f"\n🕐 <b>Time:</b> {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC\n\n💡 <i>Recommendation based on news + ML analysis</i>"
        _send_message(text)
        logger.info('trading_alert_sent', symbol=symbol, signal=combined_signal['final_signal'], confidence=combined_signal['confidence'])

    def check_new_news(self):
        session = get_session()
        try:
            since = datetime.utcnow() - timedelta(minutes=5)
            query = session.query(NewsArticle).filter(NewsArticle.fetched_at >= since, NewsArticle.sentiment_score != None).order_by(NewsArticle.fetched_at.desc())
            ...
            for news in news_list:
                ...
                if news_analysis['urgency'] in ['high', 'medium']:
                    for symbol in news_analysis['symbols']:
                        ...
                        logger.info('news_analyzed', symbol=symbol, urgency=news_analysis['urgency'], signal=combined['final_signal'], confidence=combined['confidence'])
        except Exception as e:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='News-based trading signal analyzer with Telegram alerts')
    parser.add_argument('--symbols', type=str, default='BTCUSDT,ETHUSDT,BNBUSDT', help='Comma-separated symbols to monitor (default: BTCUSDT,ETHUSDT,BNBUSDT)')
