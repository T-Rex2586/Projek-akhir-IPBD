"""
Sistem deteksi anomali waktu nyata berbasis algoritma klasifikasi nir-supervisi (unsupervised) untuk mengklasifikasikan rekaman data finansial berkecepatan tinggi.
"""
import os
import sys
import time
...
class StreamAnomalyInference:
    ...
    def _load_model(self):
        if not os.path.exists(MODEL_PATH):
            logger.info('stream_inference_model_not_found', path=MODEL_PATH, message='Anomaly detection will use rule-based only. To train: python ml/training/train_anomaly_model.py')
            self._model = None
            return False
        try:
            self._model = joblib.load(MODEL_PATH)
            self._model_mtime = os.path.getmtime(MODEL_PATH)
            logger.info('stream_inference_model_loaded', path=MODEL_PATH, mtime=time.ctime(self._model_mtime))
            return True
    ...
    def predict_single(self, data: dict) -> bool:
        ...
        if self._model is None:
            return False
        try:
            features = np.array([[float(data.get('price', 0)), float(data.get('volume', 0)), float(data.get('price_change', 0)), float(data.get('volume_change', 0)), float(data.get('price_volatility', 0))]])
    ...
    def predict_batch(self, data_list: list) -> list:
        ...
        try:
            features = np.array([[float(d.get('price', 0)), float(d.get('volume', 0)), float(d.get('price_change', 0)), float(d.get('volume_change', 0)), float(d.get('price_volatility', 0))] for d in data_list])
